...
impl LogWatcher {
    pub fn register(filename: String) -> Result<LogWatcher, io::Error> {
        // Attempt to open a handle for the file, escalate error if needed
        ...
        // Make a reader and seek to the end of the file
        let mut reader = BufReader::new(f);
        let pos = metadata.len();
        reader.seek(SeekFrom::Start(pos)).unwrap();

        // Return success, logwatcher object
        Ok(LogWatcher {
            filename: filename,
            inode: metadata.ino(),
            pos: pos,
        ...
        F: Fn(String),
    {
        loop {
            ...
            match resp {
                Ok(len) => {
                    if len > 0 {
                        // If we read data successfully, move our position and seek ahead
                        // Call the callback with the line we read
                        self.pos += len as u64;
                        self.reader.seek(SeekFrom::Start(self.pos)).unwrap();
                        callback(line.replace("\n", ""));
                        line.clear();
                    } else {
                        // We didn't read anything... we should check if the log was rotated
                        if self.finish {
                            break;
                        } else {
